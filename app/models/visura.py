...
class VisuraView:
    ...
    def select_dettagli_fabbricato(flagstorico: bool, comune: str, foglio:str, particella: str, codiceimmobile: int) -> str:
        ...
        SELECT
            ...
            F.GEN_DESCR || COALESCE(' DEL ' || TO_CHAR(TO_DATE(F.GEN_REGIST, 'DDMMYYYY'::TEXT), 'DD/MM/YYYY'), '') || COALESCE(', NUMERO ' || F.GEN_NUMERO || '.' || F.GEN_PROGRE || '/' || F.GEN_ANNO, '') || COALESCE(', IN ATTI DAL ' || TO_CHAR(TO_DATE(F.GEN_EFF, 'DDMMYYYY'::TEXT), 'DD/MM/YYYY'), '') AS DERIVANTI_DA,
    ...
    def select_dettagli_terreno(flagstorico: bool, comune: str, foglio:str, particella: str, codiceimmobile: int) -> str:
        ...
        SELECT
            ...
            T.GEN_DESCR || COALESCE(' DEL ' || TO_CHAR(TO_DATE(T.GEN_REGIST, 'DDMMYYYY'::TEXT), 'DD/MM/YYYY'), '') || COALESCE(', NUMERO ' || T.GEN_NUMERO || '.' || T.GEN_PROGRE || '/' || T.GEN_ANNO, '') || COALESCE(', IN ATTI DAL ' || TO_CHAR(TO_DATE(T.GEN_EFF, 'DDMMYYYY'::TEXT), 'DD/MM/YYYY'), '') AS DERIVANTI_DA
        FROM
            CTCN.CTPARTIC T
        JOIN CTCN.COMUNI C ON
    ...
    def select_titolari(flagstorico: bool, comune: str, codiceimmobile: int, tipoimmobile: str) -> str:
        ...
        SELECT
            ...
            T.IDENTIFICA,
            T.GEN_DESCR || COALESCE(' DEL ' || TO_CHAR(TO_DATE(T.GEN_REGIST, 'DDMMYYYY'::text), 'DD/MM/YYYY'), '') || COALESCE(', NUMERO ' || T.GEN_NUMERO || '.' || T.GEN_PROGRE || '/' || T.GEN_ANNO, '') as DERIVANTI_DA
        FROM
            CTCN.CTTITOLA T
